...
2. Create TimescaleDB database `agentic-postgres-workshop` if not already created
...
### What hypertables and CAGGs are in my database?

```
Query and document the hypertables and continuous aggregates in the database.
```

### Compare Daily Average Temperature Query Execution Time

```
Compare performance of queries with and without continuous aggregates.
```

### Number of records in daily sensor data

```
Count and analyze the volume of data in the sensor readings.
```

## 4. Data Retention Experiment

Experiment with data retention policies:
...
```
Check the chunk sizes and suggest optimizations for better performance.
```